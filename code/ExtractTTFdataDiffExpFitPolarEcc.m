%% ExtractTTFdataWatsonModelFitPolarEcc
%
...
% 
% Data are split into 8 polar angle divisions, with 4 of the divisions
% centered around the meridian, and 4 in between the meridians
...
eccenDivs = [0 22.5;22.5 90]; % eccentricity bins
...
for ss = 1:2
    for dd = 1:3
        for ee = 1:size(eccenDivs,1)
            eccenRange = eccenDivs(ee,:);
            for pp = 1:size(polarDivs,1)
                polarRange = polarDivs(pp,:);
                ...
                if pp == 5
                    areaIdx = (vArea==1) .* (eccenMap > eccenRange(1)) .* (eccenMap < eccenRange(2)) .* (abs(polarMap) > polarRange(1)) .* (abs(polarMap) < polarRange(2));
                else
            ...
            for ff = 1:nFreqs
                ...
                figure(ss+(3*pp))
                subplot(3,length(eccenDivs),ee+(dd-1)*(length(eccenDivs)));
                ...
                Boot_Vals(ss,dd,ee,pp,:,:) = BootVals;

                %fit bootstrapped values with R2 values
                for xx = 1:1000
                    yy = BootVals(:,xx)';
                    [wFit,yFit,yFit2,pBoot(ss,dd,ee,pp,xx,:)] = fitExp(w,yy);
                    maxBoot(ss,dd,ee,pp,xx,:) = max(yFit);
                   if max(yFit)<0
                   ...
                   if length(temp)>1
                    ...
                    peakFreqBoot(ss,dd,ee,pp,xx,:) = temp;
                    R = corrcoef(yy,yFit2);
                    r2Boot(ss,dd,ee,pp,xx) = R(1,2)^2;
                    yFitBoot(ss,dd,ee,pp,xx,:) = yFit;
    ...
    %                 xlabel(sprintf('p1 = %.3g, p2 = %.3g, p3 = %.3g',squeeze(squeeze(squeeze(pBoot(ss,dd,ee,pp,xx,:))))))
...
save_file = [localSaveDir '/expFitsPolarEcc.mat'];
save(save_file,'data','eccenDivs','polarDivs','Boot_Vals','peakFreqBoot','maxBoot','pBoot','r2Boot','yFitBoot');
