function [ midgetRFDensitySqDegVisual ] = watsonMidgetRFDensityByEccenDegVisual(polarAngle)
...
watsonFunc = @(posDeg) 2 * dcZero .* ...                       % The number of mRFs at the fovea (which is twice the cone density)
    ( (a*((1+posDeg./r2)).^-2) + (1-a).*exp(-1.*posDeg./re) ) .* ...  % The number of receptive fields as a function of eccentricity
    ((1+posDeg./rm).^-1);                                                    % The midget fraction at each eccentricity

midgetRFDensitySqDegVisual = @(posDeg) zeroOpticDiscPoints(watsonFunc(posDeg), posDeg, polarAngle);
