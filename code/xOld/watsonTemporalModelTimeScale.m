function y = watsonTemporalModelTimeScale(frequenciesToModel, params, params_centerAmplitudeIn, filterOrder)
...
%                                   the surround filter
%                           omega - multiplier of the time base
%   params_centerAmplitude - Scalar. Optional. The fifth parameter of the
%                           model.
...
%{
    ...
    params = [2 2 1 0];
    y = watsonTemporalModelTimeScale(freqHz,params);
    semilogx(freqHz,y,'-k');    
%}
%{
    ...
    myObj = @(p) sqrt(sum((scaledBOLDresponse-watsonTemporalModelTimeScale(stimulusFreqHz,p)).^2));
    x0 = [4 2 1 0];
    params = fmincon(myObj,x0,[],[]);
    semilogx(stimulusFreqHzFine,watsonTemporalModelTimeScale(stimulusFreqHzFine,params).*splineInterpolatedMax+minBOLD,'-k');
    hold on
...
params_omega = params(4);

% The omega parameter is a kludge to adjust the spacing of the modeled
% frequencies to "squeeze" or "expand" the shape of the returned response
omegaScale = params_omega.*(log10(frequenciesToModel)-mean(log10(frequenciesToModel)));
frequenciesToModel = 10.^(log10(frequenciesToModel)+omegaScale);
